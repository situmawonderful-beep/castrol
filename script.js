...
function applyConfig() {
  ...
  if (CONFIG.colors.primary)    root.style.setProperty('--lav-deep', CONFIG.colors.primary);
  if (CONFIG.colors.secondary)  root.style.setProperty('--lav',      CONFIG.colors.secondary);
  if (CONFIG.colors.gold)       root.style.setProperty('--gold',     CONFIG.colors.gold);
  if (CONFIG.colors.background) root.style.setProperty('--cream',    CONFIG.colors.background);

  document.querySelectorAll('.logo-name, .f-name').forEach(el => el.textContent = CONFIG.business.name);
  ...
  if (mapIframe && CONFIG.mapUrl) mapIframe.src = CONFIG.mapUrl;

  document.querySelectorAll('a[href*="wa.me"]').forEach(el => {
    el.href = `https://wa.me/${CONFIG.business.whatsapp}`;
  });
}
